...
def HandleHello(self, data):
    ...
    toSend.set("PacketData", "domainPartition.subDomain", "bfwest-dedicated")
    toSend.set("PacketData", "curTime", currentTime)
    toSend.set("PacketData", "activityTimeoutSecs", "3600")  # We could let idle clients disconnect here automatically?
...
def SendMemCheck(self):
    ...
    if self.CONNOBJ.IsUp:
        Packet(toSend).send(self, "fsys", 0xC0000000, 0)
...
def HandleGoodbye(self, data):
    ...
    if reason == "GOODBYE_CLIENT_NORMAL":
        if message.replace("%3d", "=") == "n/a":
            self.logger.new_message("[" + self.ip + ":" + str(self.port) + '][fsys] Client disconnected because of unknown error!', 2)
        else:
